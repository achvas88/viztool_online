    ...
    $.widget("ui.sexyselect", {
        ...
        _createFilter: function()
        {
            ...
            var filterWrapper = $(self.el.d)
                ...
                var filterHeaderClear = $(self.el.d).css({'clear':'both','height':'0px'});
                var filterWrapText = $(self.el.s)
                                        .addClass('ui-sexyselect-filter-header-text')
                                        .text('filter')
                                        .css('font-size','0.9em');
                                    
                var filterWrapImg = $(self.el.s)
                                    .addClass('ui-icon')
        ...
        addItem : function(inputText, index)
        {
            ...
            if(index !== undefined)
            {
                var options = self.elements();
                self.element.html('');
                $.each(options,function(idx,i)
                {
                    if(idx == index)
        ...
        _syncItems : function()
        {
            ...
            var filterText = $('#' + initElem.attr('id') + '_filterInput').val();//.trim();
            if(filterText !== undefined) 
                filterText.trim();
            else 
                filterText = null;
            ...
            if((self.totalItems() > 1 || self.options.connectedList.connection != null) && !self.options.autoSort)
            {
                ...
                itemsWrapper.sortable({ 
                    ...
                    update: function(event, ui) {
                        ...
                        var firstItem = $(ui.item).attr('oitem');
                        if(itemsWrapper.attr('id') != firstList)
                        {
                            ...
                            {
                                $('#' + firstItem).sexyselect('removeItem',self._getText($(ui.item)));
                                self.addItem(self._getText($(ui.item)),$(ui.item).index());
                            }
                        }
                        else
                            self._syncItems();
                        ...
                        {
                            var i = $(itm).find('span:eq(2)'); //$($(itm).children('span').get(2))
                            var text = i.html();
                ...
                if(self.options.connectedList.connection != null)
                {
                    if(self.options.connectedList.connection.indexOf('#') == -1)
                        self.options.connectedList.connection = '#' + self.options.connectedList.connection;
        ...
        {
            var self = this;
            var wrap = $(self.el.u).css({'list-style-type':'none', 'padding' : '0' , 'margin' : '0','cursor':'pointer' }).append();
            var item = $(self.el.li)
